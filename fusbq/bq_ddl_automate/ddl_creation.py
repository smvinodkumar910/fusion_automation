...
from fusbq.bq_ddl_automate.Fusion_raw_view_create import create_raw_views
...
def create_raw_sil_df(ddldf: pd.DataFrame, stg_table_name, raw_table_name, target_path, module: str):
    pc = DataSets(module)
    ...
    '''
    #generate_raw_sil(ddldf, module)


def create_dw_sil_df(ddldf: pd.DataFrame, stg_table_name, dw_table_name, target_path, module: str):
    pc = DataSets(module)
    ddldf['source_table_schema'] = pc.STG_SCHEMA
    ddldf['source_table_name'] = stg_table_name
    ddldf['target_schema'] = pc.RAW_SCHEMA
    ...
    '''
    #generate_dw_sil(ddldf,module)


def create_del_tables(oradf, stg_del_table_name, raw_table_name, dw_table_name, target_path, module: str):
    pc = DataSets(module)
    ...
    create = 'CREATE TABLE IF NOT EXISTS `project_name.' + pc.STG_SCHEMA + '.' + stg_del_table_name + '`\n(\n'
    ...
    #create = 'CREATE TABLE IF NOT EXISTS `project_name.' + pc.DW_SCHEMA + '.' + dw_table_name + '_DEL`\n(\n'
    ...
    bqschema_without_meta = deepcopy(bqschema)
    bqschema.append({
    ...
    #generate_dw_del_sil(oradf, stg_del_table_name, dw_table_name, module)
    #generate_dw_del_plp(dw_table_name, module)
    #generate_raw_del_plp(oradf, stg_del_table_name, raw_table_name, module)
...
def create_bq_ddl(oradf: pd.DataFrame, table_name: str, stg_table_name: str, raw_table_name: str, dw_table_name: str,
    ...
    #os.makedirs(os.path.join(target_path, 'ADHOC_JSON'), exist_ok=True)
    #print('created folders in '+target_path)
    ...
    oradf['raw_view_columns'] = raw_new_list
    ...
    colstrraw = createraw[:-2] + '\n)\nPARTITION BY DATE(_metadata_update_time) OPTIONS (labels=[label_1]);'
    ...
    colstrhist = createhist[:-2] + '\n)\nPARTITION BY DATE(_metadata_insert_time) OPTIONS (labels=[label_1]);'
    
    # DW table ddl creation
    #createdw = 'CREATE TABLE IF NOT EXISTS `project_name.' + pc.DW_SCHEMA + '.' + dw_table_name + '`\n(\n'
    ...
    blob = databucket.blob(module.lower() + pc.SCHEMA_OBJECT_PATH + stg_table_name  +  stg_table_name + '.json')
    blob.upload_from_string(json.dumps(bqstgschema_without_meta, indent=2))
    ...
    create_del_tables(oradf, stg_del_table_name, raw_table_name, dw_table_name, target_path, module)
    create_raw_views(oradf, raw_table_name, target_path, module)
