...
def create_stg_tables(table_type: str):
    ...
    bqclient = bigquery.Client(data_project_name)
    ...
    for a in blob_list:
        ...
        qry = "select MODULE from helix-data-dev.BQ_CTL_METADATA.FUSION_TO_BQ_PVO_DTL \
              where stg_table_name='{}'".format(table_name)
        job = bqclient.query(qry)
        for row in job.result():
            module = row[0]
        create_table_api(table_name, table_type,module)
...
def create_hist_tables(table_type: str):
    ...
    for a in blob_list:
        ...
        table_name = path[-1][0:-4]
        qry = "select MODULE from helix-data-dev.BQ_CTL_METADATA.FUSION_TO_BQ_PVO_DTL \
              where hist_table_name='{}'".format(table_name)
        job = bqclient.query(qry)
        for row in job.result():
            module = row[0]
        create_table_api(table_name, table_type,module)
...
def create_raw_tables(table_type: str):
    ...
    for a in blob_list:
        ...
        table_name = path[-1][0:-4]
        qry = "select MODULE from helix-data-dev.BQ_CTL_METADATA.FUSION_TO_BQ_PVO_DTL \
              where raw_table_name='{}'".format(table_name)
        job = bqclient.query(qry)
        for row in job.result():
            module = row[0]
        create_table_api(table_name, table_type,module)
...
@task(task_id='create_raw_views')
def create_raw_views():
    from google.cloud import storage,bigquery
    client = storage.Client(project=project_name)
    bqclient = bigquery.Client(data_project_name)
    bucket = client.get_bucket(bucket_name)
    blob_list = bucket.list_blobs(prefix=temp_path + 'RAW_VIEWS/')
    for a in blob_list:
        blob = bucket.get_blob(a.name)
        script = blob.download_as_string()
        print(a.name)
        ...
        for row in job.result():
            module = row[0]
        # Creating RAW procs
        create_procs(str(script, 'UTF-8'),module)
...
with models.DAG(
    ...
    create_raw_views = create_raw_views()
    ddl_create_bip = ddl_create_bip()
...
prep_step >> DDL_CREATE >> ddl_create_bip >> [create_stg_tables, create_raw_tables, create_hist_tables,
                            create_stg_del_tables] >> tables_creation_check >> create_raw_views >> post_job_step >> move_ddl_files
