...
def pre_batch(**kwargs):
    ...
    set DAG_RUN_ID = '{v_dag_run_id}' where IS_ACTIVE = true;"""
...
def post_batch(**kwargs):
    ...
    context = kwargs
    print(context)
    query_stmt = """CALL `{v_project_id}.{v_dataset_name}.BQ_POST_BATCH_CTL_PROC_LOGGING`('{V_Batch_Name}');"""
...
def post_batch_fail(**kwargs):
    ...
    context = kwargs
    print(context)
    query_stmt = """CALL `{v_project_id}.{v_dataset_name}.BQ_POST_BATCH_FAIL_CTL_PROC_LOGGING`('{V_Batch_Name}');"""
...
def create_dynamic_tasks():
    ...
    sql = 'select table_id, datastore_name, stg_table_name, raw_sil_proc ' \
          'from {}.{}.FUSION_TO_BQ_PVO_DTL ' \
          'WHERE MODULE=\'FA\' ORDER BY TABLE_ID'.format(DATA_PROJECT_ID, DATASET_NAME)
    ...
    for a in output_dict:
        ...
        stg_table_load.set_downstream(raw_sil_proc_exe)
