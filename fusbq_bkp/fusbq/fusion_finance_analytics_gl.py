...
CONTROL_SCHEDULE_TABLE_NAME = task_configs["controlSchduleTableName"]
...
STG_COMPLETION_CHECK = EmptyOperator(
...
@task(task_id="check_any_ddl_required")
def ddl_create_check(**kwargs):
    try:
        client = bigquery.Client(DATA_PROJECT_ID)
        sql = 'select count(1) as CNT ' \
              'from {}.{}.FUSION_TO_BQ_PVO_DTL ' \
              'WHERE RUN_DDL_FLAG=true AND MODULE = \'GL\' '.format(DATA_PROJECT_ID, DATASET_NAME)

        data = client.query(sql).result()
        for a in data:
            rec_cnt = a.CNT
        kwargs['ti'].xcom_push(key='ddl_count', value=rec_cnt)
...
def branch_func(**kwargs):
    ti = kwargs['ti']
    count = ti.xcom_pull(key='ddl_count', task_ids='check_any_ddl_required')
    if count > 0:
        return "ddl_automation_dag_trigger"
    else:
        return None
...
def create_dynamic_tasks():
    client = bigquery.Client(DATA_PROJECT_ID)
    sql = 'select table_id, datastore_name, stg_table_name, hist_table_name, raw_sil_proc ' \
          'from {}.{}.FUSION_TO_BQ_PVO_DTL ' \
    ...
    for a in output_dict:
        stg_table_load = GCSToBigQueryOperator(
            task_id='fusion_bq_stg_' + a['table_id'],
            bucket=DATA_BUCKET,
            source_objects=[
                INBOUND_FOLDER_PATH + '/' + a['file_name'] + '*.csv'],
            schema_fields=None,
            schema_object=FUSION_SCHEMA + '/' + a['stg_table_name'] + '.json',
            destination_project_dataset_table=DATA_PROJECT_ID + "." + BQ_STG_DATASET_NAME + '.' + a['stg_table_name'],
            source_format='CSV',
            create_disposition='CREATE_IF_NEEDED',
            skip_leading_rows=1,
            autodetect=False,
        ...
        hist_proc_exe = BigQueryExecuteQueryOperator(
            task_id="hist_" + a['table_id'],
            sql="CALL `" + DATA_PROJECT_ID + ".helix_stg.STG_HIST_LOAD_PROC`('" + a['stg_table_name'] + "')",
        ...
        raw_sil_proc_exe = BigQueryExecuteQueryOperator(
            task_id="raw_sil_proc_exe_" + a['table_id'],
            sql="CALL `" + DATA_PROJECT_ID + ".helix_raw." + a['raw_sil_proc'] + "`(10001)",
            use_legacy_sql=False,
            on_failure_callback=task_failure,
            on_success_callback=task_success,
            retries=0,
        ...
        stg_table_load.set_downstream(hist_proc_exe)
        hist_proc_exe.set_downstream(raw_sil_proc_exe)
        raw_sil_proc_exe.set_downstream(STG_COMPLETION_CHECK)


if 'prod' in pc.ENVIRONMENT_NAME:
    ...
    FileExtract.set_downstream(FILE_COPY_UAT)

ddl_create_check = ddl_create_check()
branch_func = branch_func()

# Taskflow starts
PRE_BATCH_STEP >> [FileExtract, ddl_create_check]
ddl_create_check.set_downstream(branch_func)
branch_func.set_downstream(ddl_automation_dag_trigger)
FileExtract.set_downstream(join_task)
ddl_automation_dag_trigger.set_downstream(join_task)
join_task.set_downstream(step1)
