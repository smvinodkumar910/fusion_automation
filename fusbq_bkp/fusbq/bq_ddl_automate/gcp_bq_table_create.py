...
def create_table_api(table_name: str, table_type: str):
    if table_type == 'STG':
    ...
    schema = []
    for a in schema_dtl:
        if a['name'] == 'DELETE_FLAG':
            schema.append(bigquery.SchemaField(a['name'], a['type'], a['mode'], a['default_value_expression']))
        else:
    ...
    if bq_table_exists(table_id):
        ...
        if table.schema == schema:
            ...
            replace_bq_table_schema(table_id, table.schema, schema)
    else:
...
def replace_bq_table_schema(table_id: str, old_schema, new_schema):
    """
    ...
    merged_df = pd.merge(old_schema, new_schema, how='left',
                         left_on=['old_columns_name'],
                         right_on=['new_columns_name'])

    merged_df['insert_cols'] = merged_df.apply(lambda
                                                   x: x.old_columns_name if x.new_column_type == x.old_column_type else 'CAST(' + x.old_columns_name + ' as ' + x.new_column_type + ')',
                                               axis=1)
